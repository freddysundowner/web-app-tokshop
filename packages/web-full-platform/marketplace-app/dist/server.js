...
function registerOrderRoutes(app2) {
  ...
  app2.get("/api/orders", async (req, res) => {
    try {
      ...
      if (req.query.marketplace) {
        queryParams.set("marketplace", req.query.marketplace);
      }
  ...
  app2.post("/api/orders/cancel/order", async (req, res) => {
    try {
      ...
      const response = await fetch3(`${BASE_URL}/orders/cancel/order`, {
        method: "POST",
        headers,
        body: JSON.stringify({
          order,
  ...
  app2.post("/api/orders/cancel/approve", async (req, res) => {
    try {
      ...
      const payload = {
        order: item || order,
        // Use item ID if present, otherwise order ID
        action: "approve",
  ...
  app2.post("/api/orders/cancel/decline", async (req, res) => {
    try {
      ...
      const payload = {
        ...
        type: item ? "item" : "order",
        initiator: "seller",
        description: description || (item ? "Item cancellation declined by seller" : "Order cancellation declined by seller")
      };
...
async function initializeFirebaseAdmin() {
  ...
  try {
    ...
    const response = await fetchWithTimeout(`${BASE_URL}/settings`, 5e3);
    if (!response.ok) {
...
function registerShowRoutes(app2) {
  ...
  app2.put("/api/rooms/features/:roomId", async (req, res) => {
    try {
  ...
  app2.put("/api/rooms/:id", async (req, res) => {
    try {
      ...
      const data = await response.json();
      console.log("\u{1F4CB} PUT response data structure:", Object.keys(data));
      res.json(data);
    } catch (error) {
  ...
  app2.get("/api/rooms", async (req, res) => {
    try {
      ...
      if (req.query.live !== void 0) params.push(`live=${req.query.live}`);
      if (req.query.featured !== void 0) params.push(`featured=${req.query.featured}`);
      if (req.query.ownerUsername !== void 0) params.push(`ownerUsername=${req.query.ownerUsername}`);
...
var socialAuthSchema = z2.object({
  ...
  providerToken: z2.string().optional(),
  // OAuth provider token (Google accessToken or Apple identityToken) for backend decoding
...
var tokshopOrderSchema = z2.object({
  ...
  items: z2.array(z2.object({
    ...
    order_reference: z2.string().optional(),
    status: z2.string().optional()
  })).optional(),
  // Financial fields
  ...
  service_fee: z2.number().optional(),
  // Commission/platform fee charged by the platform
  stripe_fees: z2.number().optional(),
...
var productFormSchema = z2.object({
  ...
  list_individually: z2.boolean().optional().default(false),
  ...
  acceptsOffers: z2.boolean().optional().default(false),
...
var helpArticleSchema = z2.object({
  _id: z2.string().optional(),
  title: z2.string().min(1, "Title is required"),
  slug: z2.string().min(1, "Slug is required"),
  excerpt: z2.string().min(1, "Excerpt/description is required"),
  content: z2.string().min(1, "Content is required"),
  category: z2.enum(["general", "seller", "buyer", "payments", "shipping", "other"]).default("general"),
  published: z2.boolean().default(true),
  order: z2.number().default(0),
  createdAt: z2.string().optional(),
  updatedAt: z2.string().optional()
});
var createHelpArticleSchema = helpArticleSchema.omit({
  _id: true,
  createdAt: true,
  updatedAt: true
});
var updateHelpArticleSchema = helpArticleSchema.partial().omit({
  _id: true,
  createdAt: true,
  updatedAt: true
});
...
function registerShippingRoutes(app2) {
  ...
  app2.get("/api/shipping/metrics", async (req, res) => {
    try {
      ...
      const response = await fetch4(`${BASE_URL}/orders/shipments/metrics/${userId}${queryString}`, {
        method: "GET",
        headers
      });
      if (!response.ok) {
    ...
    try {
      ...
      if (estimate && typeof estimate === "object") {
        ...
        if (hasError && errorMessage) {
          ...
          if (lowerMsg.includes("identical") || lowerMsg.includes("same address") || lowerMsg.includes("same location") || lowerMsg.includes("shipping to yourself")) {
  ...
  app2.post("/api/shipping/profiles/buy/label", async (req, res) => {
    try {
      ...
      const rates = [{
        ...
        ...req.body.estimate_data && {
          estimate_data: req.body.estimate_data
        }
      }];
  ...
  app2.post("/api/shipping/bulk-labels", async (req, res) => {
    try {
      ...
      const ordersResponse = await fetch4(`${BASE_URL}/orders?userId=${userId}`, {
        method: "GET",
        headers
      });
      if (!ordersResponse.ok) {
      ...
      for (const selectedId of orderIds) {
        ...
        rates.push({
          ...
          order: bundleIdToSend
          // Always use bundleId from order object
        });
      }
      if (rates.length === 0) {
        return res.status(400).json({
  ...
  app2.post("/api/shipping/generate/manifest", async (req, res) => {
    try {
      ...
      const response = await fetch4(`${BASE_URL}/shipping/generate/manifest`, {
        method: "POST",
        headers,
  ...
  app2.post("/api/shipping/generate/manifest/view", async (req, res) => {
    try {
      ...
      const response = await fetch4(`${BASE_URL}/shipping/generate/manifest?${params.toString()}`, {
        method: "GET",
        headers
      });
      const data = await response.json();
  ...
  app2.get("/api/shipping/generate/manifest/:manifestId", async (req, res) => {
    try {
      ...
      const params = new URLSearchParams();
      params.set("tokshow", tokshow);
      const response = await fetch4(`${BASE_URL}/shipping/generate/manifest/${manifestId}?${params.toString()}`, {
        method: "GET",
        headers
      });
      const data = await response.json();
...
var createProductSchema = z3.object({
  ...
  list_individually: z3.boolean().optional(),
  ...
  app2.post("/api/products/:userId", async (req, res) => {
    try {
      ...
      const tokshopProductData = {
        ...
        ...req.body.list_individually !== void 0 && { list_individually: req.body.list_individually },
        ...
        ...req.body.offer !== void 0 && { offer: req.body.offer },
        ...(() => {
...
var EMAIL_LOG_DIR = path.join(process.cwd(), "logs");
var EMAIL_SENT_LOG = path.join(EMAIL_LOG_DIR, "emails_sent.log");
if (!fs.existsSync(EMAIL_LOG_DIR)) {
  fs.mkdirSync(EMAIL_LOG_DIR, { recursive: true });
}
function logSentEmail(email, messageId, campaignId) {
...
function requireAdmin(req, res, next) {
  if (!req.session?.user) {
    const accessToken = req.headers["x-admin-token"] || req.headers["x-access-token"] || (req.headers["authorization"]?.startsWith("Bearer ") ? req.headers["authorization"].substring(7) : null);
      ...
      try {
        ...
        if (settingsResponse.ok) {
          ...
          if (settings?.email_from_address && email) {
            ...
            const emailSubject = (settings.seller_approval_email_subject || "\u{1F389} Your {appName} Seller Account is Approved!").replace(/{appName}/g, appName);
            const emailBody = (settings.seller_approval_email_body || "Congratulations! Your seller account on {appName} has been approved.\n\nYou can now:\n- List products for sale\n- Host live shopping shows\n- Run auctions and giveaways\n- Start earning on the platform\n\nGet started by visiting your Seller Hub.").replace(/{appName}/g, appName).replace(/{name}/g, data.data?.firstName || "there");
            const bodyHtml = emailBody.split("\n").map((line) => line.trim() ? `<p style="margin: 0 0 15px; font-size: 16px; line-height: 1.6; color: #333333;">${line}</p>` : "").join("");
            const emailHtml = `
<!DOCTYPE html>
<html>
<head>
...
\xA9 ${(/* @__PURE__ */ new Date()).getFullYear()} ${appName}. All rights reserved.
  ...
  app2.get("/api/admin/transactions", requireAdmin, async (req, res) => {
    try {
      ...
      const accessToken = req.headers["x-admin-token"] || req.headers["x-access-token"] || req.session?.accessToken || (req.headers["authorization"]?.startsWith("Bearer ") ? req.headers["authorization"].substring(7) : null);
  ...
  app2.get("/api/themes", requireAdmin, async (req, res) => {
    try {
      const accessToken = req.session.accessToken;
      if (!accessToken) {
        return res.status(401).json({
  ...
  app2.post("/api/themes", requireAdmin, checkDemoMode, async (req, res) => {
    try {
      const accessToken = req.session.accessToken;
      if (!accessToken) {
        return res.status(401).json({
  ...
  app2.post("/api/themes/upload-logo", requireAdmin, checkDemoMode, upload.single("logo"), async (req, res) => {
    try {
      const accessToken = req.session.accessToken;
      if (!accessToken) {
        return res.status(401).json({
  ...
  app2.post("/api/themes/upload-header-logo", requireAdmin, checkDemoMode, upload.single("logo"), async (req, res) => {
    try {
      const accessToken = req.headers["x-admin-token"] || req.headers["x-access-token"] || req.session.accessToken || (req.headers["authorization"]?.startsWith("Bearer ") ? req.headers["authorization"].substring(7) : null);
      if (!accessToken) {
        return res.status(401).json({
  ...
  app2.post("/api/themes/upload-resource", requireAdmin, checkDemoMode, upload.single("file"), async (req, res) => {
    try {
      const accessToken = req.headers["x-admin-token"] || req.headers["x-access-token"] || req.session.accessToken || (req.headers["authorization"]?.startsWith("Bearer ") ? req.headers["authorization"].substring(7) : null);
      if (!accessToken) {
        return res.status(401).json({
  ...
  app2.post("/api/admin/categories", requireAdmin, checkDemoMode, upload.array("images", 5), async (req, res) => {
    try {
      ...
      if (commission !== void 0 && commission !== "") {
        formData.append("commission", commission);
      }
      if (commission_enabled !== void 0 && commission_enabled !== "") {
...
`;
  ...
  app2.get("/api/admin/articles/published/articles", requireAdmin, async (req, res) => {
    try {
      const accessToken = req.session.accessToken;
      if (!accessToken) {
        return res.status(401).json({
  ...
  app2.get("/api/admin/articles/published/articles/:slug", requireAdmin, async (req, res) => {
    try {
      const accessToken = req.session.accessToken;
  ...
  app2.get("/api/admin/articles/:id", requireAdmin, async (req, res) => {
    try {
      const accessToken = req.session.accessToken;
      const { id } = req.params;
      if (!accessToken) {
  ...
  app2.post("/api/admin/articles", requireAdmin, checkDemoMode, async (req, res) => {
    try {
      const accessToken = req.session.accessToken;
      if (!accessToken) {
        return res.status(401).json({
  ...
  app2.put("/api/admin/articles/:id", requireAdmin, checkDemoMode, async (req, res) => {
    try {
      const accessToken = req.session.accessToken;
      const { id } = req.params;
      if (!accessToken) {
  ...
  app2.delete("/api/admin/articles/:id", requireAdmin, checkDemoMode, async (req, res) => {
    try {
      const accessToken = req.session.accessToken;
      const { id } = req.params;
      if (!accessToken) {
  ...
  app2.get("/api/articles/published/:slug", async (req, res) => {
    try {
      const { slug } = req.params;
      const url = `${BASE_URL}/articles/published/articles/${slug}`;
      const response = await fetch(url, {
  ...
  app2.post("/api/admin/brevo/send-campaign", requireAdmin, checkDemoMode, async (req, res) => {
    try {
  ...
  app2.post("/api/admin/brevo/send-batch", requireAdmin, checkDemoMode, async (req, res) => {
    try {
      ...
      const replyTo = replyToEmail ? { email: replyToEmail } : void 0;
      const messageVersions = recipients.map((recipient) => ({
        to: [{
          email: recipient.email,
          name: recipient.name || recipient.email.split("@")[0]
        }],
  ...
  app2.post("/api/admin/brevo/send-test", requireAdmin, async (req, res) => {
    try {
...
function registerSettingsRoutes(app2) {
  app2.get("/api/settings/keys", async (req, res) => {
    try {
  ...
  app2.get("/api/public/themes", async (req, res) => {
    try {
      const url = `${BASE_URL}/themes`;
  ...
  app2.get("/api/settings/full", async (req, res) => {
    try {
      const accessToken = req.session?.accessToken;
      if (!accessToken) {
        return res.status(401).json({
...
var BASE_URL2 = process.env.BASE_URL || "https://api.tokshoplive.com";
...
function registerContentRoutes(app2) {
  app2.get("/api/content/:pageType", async (req, res) => {
    try {
      ...
      const response = await fetch(`${BASE_URL2}/content/${pageType}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json"
        }
  ...
  app2.put("/api/admin/content/:pageType", requireContentAdmin, async (req, res) => {
    try {
      const session2 = req.session;
      const pageTypeValidation = pageTypeEnum.safeParse(req.params.pageType);
      if (!pageTypeValidation.success) {
      ...
      const token = session2?.accessToken || req.headers["x-access-token"] || (req.headers["authorization"]?.startsWith("Bearer ") ? req.headers["authorization"].substring(7) : null);
      if (token) {
        headers["Authorization"] = `Bearer ${token}`;
      }
      const response = await fetch(`${BASE_URL2}/content/${pageType}`, {
  ...
  app2.post("/api/admin/content/:pageType/reset", requireContentAdmin, async (req, res) => {
    try {
      const session2 = req.session;
      const pageTypeValidation = pageTypeEnum.safeParse(req.params.pageType);
      if (!pageTypeValidation.success) {
      ...
      const response = await fetch(`${BASE_URL2}/content/${pageType}/reset`, {
        method: "POST",
        headers,
        body: JSON.stringify({})
      });
...
// ../shared-backend/server/routes/offers.ts
import fetch11 from "node-fetch";
function registerOfferRoutes(app2) {
  app2.post("/api/offers", async (req, res) => {
    try {
      ...
      const response = await fetch11(`${BASE_URL}/offers`, {
        method: "POST",
        headers,
        body: JSON.stringify(req.body)
      });
  ...
  app2.get("/api/offers", async (req, res) => {
    try {
      ...
      if (req.query.tokshowId) {
        queryParams.set("tokshowId", req.query.tokshowId);
      }
      if (req.query.user) {
        queryParams.set("user", req.query.user);
      }
      if (req.query.role) {
        queryParams.set("role", req.query.role);
      }
      if (req.query.page) {
        queryParams.set("page", req.query.page);
      }
  ...
  app2.post("/api/offers/counter", async (req, res) => {
    try {
      ...
      const response = await fetch11(`${BASE_URL}/offers/counter`, {
        method: "POST",
        headers,
        body: JSON.stringify(req.body)
      });
  ...
  app2.post("/api/offers/accept", async (req, res) => {
    try {
      ...
      const response = await fetch11(`${BASE_URL}/offers/accept`, {
        method: "POST",
        headers,
        body: JSON.stringify(req.body)
      });
  ...
  app2.post("/api/offers/reject", async (req, res) => {
    try {
      ...
      const response = await fetch11(`${BASE_URL}/offers/reject`, {
        method: "POST",
        headers,
        body: JSON.stringify(req.body)
      });
  ...
  app2.post("/api/offers/cancel", async (req, res) => {
    try {
      ...
      const response = await fetch11(`${BASE_URL}/offers/cancel`, {
        method: "POST",
        headers,
        body: JSON.stringify(req.body)
      });
...
// ../shared-backend/server/routes/contact.ts
function registerContactRoutes(app2) {
  app2.post("/api/contact", async (req, res) => {
    try {
      ...
      const supportEmail = settings.support_email && settings.support_email.trim() !== "" ? settings.support_email : settings.email_from_address;
      const appName = settings.app_name || "Our Platform";
...
var BASE_URL3 = process.env.BASE_URL || "https://api.tokshoplive.com";
router.get("/:id", async (req, res) => {
  try {
    const token = req.headers.authorization?.replace("Bearer ", "");
    const auctionId = req.params.id;
    console.log("Fetching auction details for:", auctionId);
    const response = await fetch12(`${BASE_URL3}/auction/${auctionId}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
router.post("/bid", async (req, res) => {
  try {
    ...
    const response = await fetch12(`${BASE_URL3}/auction/bid`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
router.put("/bid/:id", async (req, res) => {
  try {
    ...
    const response = await fetch12(`${BASE_URL3}/auction/bid/${auctionId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`
...
async function setupVite(app2, server) {
  ...
  const clientRoot = path2.resolve(process.cwd(), "client");
  console.log("[Vite] Using client root:", clientRoot);
  const appRoot = process.cwd();
  const viteConfigPath = path2.resolve(appRoot, "vite.config.ts");
...
// ../shared-backend/server/services/socket-listener.ts
import { io } from "socket.io-client";
var BASE_URL4 = process.env.BASE_URL || "";
var SocketListener = class {
  socket = null;
  monitoredRooms = /* @__PURE__ */ new Set();
  async initialize() {
    if (this.socket) {
    ...
    try {
      console.log(`\u{1F50C} Connecting backend Socket.IO client to: ${BASE_URL4}`);
      this.socket = io(BASE_URL4, {
      ...
      this.socket.on("room-ended", async (data) => {
        console.log("\u{1F4E7} ROOM-ENDED EVENT RECEIVED:", data);
        const roomId = data?.roomId || data?._id || data?.id || data;
        const isValidMongoId = roomId && typeof roomId === "string" && /^[a-f0-9]{24}$/i.test(roomId);
        if (isValidMongoId) {
          console.log(`\u{1F4E7} Show ended for room: ${roomId}`);
        } else {
      ...
      this.socket.onAny((eventName, ...args) => {
        if (eventName === "room-ended") {
          console.log("\u{1F4E8} [Backend Socket Event]:", eventName, args);
        }
      });
  ...
  monitorRoom(roomId) {
    ...
    console.log("\u{1F440} Starting to monitor room:", roomId);
  ...
  stopMonitoringRoom(roomId) {
    ...
    console.log("\u{1F6D1} Stopping monitoring of room:", roomId);
  ...
  disconnect() {
    if (this.socket) {
      console.log("\u{1F50C} Disconnecting backend Socket.IO client...");
