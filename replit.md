...
- **Proof-of-Purchase Video Recording**: Platform continuously buffers/records live stream video during auctions. When an auction ends with a winner, the system automatically captures the last 5-10 seconds of video before the winning bid. This video clip is permanently attached to the order as proof of what the buyer won, showing the exact item auctioned and host's description at the moment of sale. Both buyer and seller can reference this video for dispute resolution and transparency. Similar to Whatnot's implementation, this protects both parties by providing immutable evidence of the transaction. Recording is handled via LiveKit's egress capabilities with the video stored and associated with the order record in the database.
