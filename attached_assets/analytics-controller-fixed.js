...
exports.getSellerAnalytics = async (req, res) => {
    try {
        // FIX: the endpoint is called as /analytics?userId=... (query string),
        // so read from query first. Fall back to params in case a path-style
        // route (/analytics/:userId) is ever added. Guard against an invalid /
        // missing id so we fail loudly with 400 instead of letting
        // `new mongoose.Types.ObjectId(undefined)` mint a RANDOM id that
        // silently matches no orders and returns all zeros.
        ...
        const start = req.query.startDate ? new Date(req.query.startDate) : new Date(Date.now() - 13 * DAY);
        const end = req.query.endDate ? new Date(req.query.endDate) : new Date();
        const startMs = start.getTime();
        const endMs = end.getTime();

        // ----- orders (sales = subtotal, earnings stored; tokshow null = marketplace) -----
        const orders = await orderModel
            ...
            .select("subtotal earnings tokshow customer createdAt")
            .lean();
        ...
        for (let i = 0; i < windowDays; i++) {
            ...
            buckets.set(key, {
                ...
                showOrders: 0, mpOrders: 0, _showB: new Set(), _mpB: new Set(), lives: 0,
            });
        }

        let showSales = 0, mpSales = 0, showEarnings = 0, mpEarnings = 0, showOrders = 0, mpOrders = 0;
        const allB = new Set(), showB = new Set(), mpB = new Set();

        for (const o of orders) {
            const b = buckets.get(dayKey(new Date(o.createdAt).getTime()));
            const sales = Number(o.subtotal) || 0;
            const earn = Number(o.earnings) || 0;
            const buyer = o.customer ? String(o.customer) : null;
            const isShow = !!o.tokshow;
            if (isShow) {
                showSales += sales; showEarnings += earn; showOrders++;
                if (buyer) { showB.add(buyer); allB.add(buyer); }
                if (b) { b.showSales += sales; b.showEarnings += earn; b.showOrders++; if (buyer) b._showB.add(buyer); }
            } else {
                mpSales += sales; mpEarnings += earn; mpOrders++;
                if (buyer) { mpB.add(buyer); allB.add(buyer); }
                if (b) { b.mpSales += sales; b.mpEarnings += earn; b.mpOrders++; if (buyer) b._mpB.add(buyer); }
            }
        }
        ...
        const daily = orderKeys.map((k) => {
            ...
            return {
                ...
                showOrders: b.showOrders, mpOrders: b.mpOrders,
                showBuyers: b._showB.size, mpBuyers: b._mpB.size, lives: b.lives,
            };
        });

        const sales = showSales + mpSales, earnings = showEarnings + mpEarnings, orderCount = showOrders + mpOrders;
        ...
        return res.json({
            ...
            totals: {
                ...
                aov: {
                    ...
                    show: showOrders ? showSales / showOrders : 0,
                    marketplace: mpOrders ? mpSales / mpOrders : 0,
                },
