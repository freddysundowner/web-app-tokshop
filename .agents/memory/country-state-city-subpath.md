...
**Build gotcha (direct vs transitive dep):** the sub-path import only works if `country-state-city` is a DIRECT dependency of the app that imports it (e.g. `marketplace-app`). If it is only present transitively (pulled by `react-country-state-city`), npm may nest it under `react-country-state-city/node_modules/...` on a fresh install, so a top-level `import ... from 'country-state-city/lib/country'` cannot be resolved and the production `vite build` (Rollup) fails with "failed to resolve import". Dev (esbuild) and a hoisted local install can mask this. Fix: declare `country-state-city` in the importing app's own `package.json` and sync its lockfile.
